...
class ServiceBoundaryAnalyzer:
    ...
    async def detect_services(self, repository: Repository, session: AsyncSession) -> List[Service]:
        ...
        for project in projects:
            ...
            try:
                ...
                if await asyncio.to_thread(project_path.exists):
                    content = await asyncio.to_thread(project_path.read_text, encoding='utf-8')
            ...
            try:
                ...
                if await asyncio.to_thread(project_path_obj.exists):
                    ...
                    for startup_file in startup_files:
                        ...
                        if await asyncio.to_thread(full_path.exists):
                            startup_content = await asyncio.to_thread(full_path.read_text, encoding='utf-8')
    ...
    async def _detect_library_service(
        ...
        if not settings.detect_library_services:
        ...
        try:
            ...
            if symbol_count < settings.min_library_symbols:
                logger.info(
                    f"   Library {project.name} has only {symbol_count} symbols "
                    f"(threshold: {settings.min_library_symbols}) - SKIPPED"
                )
                return None
