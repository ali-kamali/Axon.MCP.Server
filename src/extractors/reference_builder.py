    ...
    async def build_all_references(self, repository_id: int) -> int:
        ...
        try:
            ...
            if repo.provider == SourceControlProviderEnum.AZUREDEVOPS:
                repo_path = Path(settings.repo_cache_dir).resolve() / "azuredevops" / repo.azuredevops_project_name / repo.name
            else:
                repo_path = Path(settings.repo_cache_dir).resolve() / repo.path_with_namespace.replace('/', '_').replace('\\', '_')
