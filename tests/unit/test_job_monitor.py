...
async def test_get_running_jobs(mock_session, mock_running_job):
    ...
    result_mock.scalars.return_value.all.return_value = [mock_running_job]
...
async def test_get_pending_jobs(mock_session):
    ...
    pending_job = MagicMock(spec=Job)
    pending_job.status = JobStatusEnum.PENDING
    
    result_mock = AsyncMock()
    result_mock.scalars.return_value.all.return_value = [pending_job]
    mock_session.execute.return_value = result_mock
...
async def test_get_failed_jobs(mock_session, mock_failed_job):
    ...
    result_mock = AsyncMock()
    result_mock.scalars.return_value.all.return_value = [mock_failed_job]
    mock_session.execute.return_value = result_mock
...
async def test_retry_failed_job_success(mock_session, mock_failed_job):
    ...
    with patch('src.workers.job_monitor.sync_repository') as mock_task:
...
async def test_cancel_job_success(mock_session, mock_running_job):
    ...
    with patch('src.workers.job_monitor.celery_app') as mock_celery:
...
async def test_get_job_stats(mock_session):
    ...
    def mock_execute_side_effect(*args, **kwargs):
        ...
        result_mock = AsyncMock()
        result_mock.scalars.return_value.all.return_value = jobs_by_status.get(status, [])
        return result_mock
...
async def test_cleanup_old_jobs(mock_session):
    ...
    # Create old jobs
    old_jobs = [
        MagicMock(spec=Job, status=JobStatusEnum.COMPLETED),
        MagicMock(spec=Job, status=JobStatusEnum.FAILED),
    ]
    
    result_mock = AsyncMock()
    result_mock.scalars.return_value.all.return_value = old_jobs
    mock_session.execute.return_value = result_mock
