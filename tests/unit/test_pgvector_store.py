...
async def test_store_embeddings_success(vector_store, mock_session):
    ...
    # UPDATED: Mock for batch query (uses scalars() now)
    ...
    # UPDATED: Now uses add_all instead of add
...
async def test_store_embeddings_error_handling(vector_store, mock_session):
    ...
    # UPDATED: With proper transaction management, exceptions are raised
    ...
    # UPDATED: Expect exception to be raised (proper error handling)
    with pytest.raises(Exception) as exc_info:
        await vector_store.store_embeddings(embedding_results)
    
    assert "Database error" in str(exc_info.value)
    # UPDATED: Rollback should be called on error
...
async def test_search_similar_basic(vector_store, mock_session):
    ...
    # Verify results - now returns 4-tuple (Symbol, similarity, File?, Repo?)
...
async def test_create_vector_index_ivfflat(vector_store, mock_session):
    ...
    # UPDATED: Should execute 3 statements: DROP, CREATE, and ANALYZE
...
async def test_store_multiple_embeddings(vector_store, mock_session):
    ...
    # UPDATED: Mock batch query with all chunks
    mock_chunks_result = MagicMock()
    ...
    # UPDATED: Now uses add_all with all embeddings at once
    mock_session.add_all.assert_called_once()
